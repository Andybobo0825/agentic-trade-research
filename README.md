...
這是一套把 FIFO 任務佇列、tmux runtime adapter 與財經 MCP/CLI 工具串成一條本機 agent workflow 的研究 harness。
...
目前 repo 的標準流程來源是 [`docs/standard-workflow-v1.md`](docs/standard-workflow-v1.md)。若舊對話、舊 backtest、舊策略名稱與該文件衝突，以 Standard Workflow 1.4 為準。

標準版只保留：

- 唯一有效主策略:`phase3_stability`;`phase3-dataset` 建立 point-in-time 證據,`phase3-screen` 是唯一技術決策入口。
- Phase 3 是 read-only deterministic technical filter；只有選股/篩選請求才執行 Phase 3。
...
只有選股/篩選請求才執行 Phase 3;analyze 模式直接分析指定 ticker,不執行 Phase 3。
