...
export function renderTaiwanAgentTeamMarkdown(result) {
  ...
  const lines = [
    `# Taiwan Agent Team Research Report — Standard Workflow ${result.workflowVersion || WORKFLOW_VERSION}`,
    '',
    ...
    '',
    '## Dexter-style architecture (seven agent lanes)',
    ...
    '',
    '## Auxiliary flow lens',
    `- Xiaoyu ETF overview data date: ${result.toolResults?.xiaoyu_overview?.dataDate || 'unavailable'}`,
    '- Backtest artifacts (historical audit evidence only, not an active strategy):',
    ...(result.backtests?.highlights || []).map((line) => `  - ${line}`),
    '',
